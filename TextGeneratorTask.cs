...
namespace TextAnalysis
{
    static class TextGeneratorTask
    {
        ...
        private static bool TryGetValueNextWord(Dictionary<string, string> nextWords, string[] ngram, out string exitWord)
        {
            return nextWords.TryGetValue(ngram[0], out exitWord)
                || ngram.Length >= 2 && nextWords.TryGetValue(ngram[1], out exitWord);
        }

        private static string[] ReturnNgram(string phraseBeginning)
        {
